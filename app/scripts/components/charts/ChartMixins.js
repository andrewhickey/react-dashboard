module.exports = {
  rotate_point: function(pointX, pointY, originX, originY, angle) {
    ...
    var x = Math.cos(angle) * (pointX-originX) - Math.sin(angle) * (pointY-originY) + originX || 0;
    var y = Math.sin(angle) * (pointX-originX) + Math.cos(angle) * (pointY-originY) + originY || 0;
    return {
      x: x,
      y: y
    };
  },
  pointsToPath: function(points_string){
    var points = points_string.split(/\s+|,/);
  ...
  roundPathCorners: function(path_string, radius, useFractionalRadius){
    return roundPathCorners(path_string, radius, useFractionalRadius);
  }
};
